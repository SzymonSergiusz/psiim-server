...
async def user_unlocks_mountain(mountain_id: str, current_user: Annotated[schemas.User, Depends(get_current_user)],
    ...
    for ach in not_achieved:
        ...
        if str(ach.achievement_id) == "bbd0a19c-83a9-4022-9504-234d0815a2b6":# Zdobądź wszystkie w Sudetach
            # Sudety = ["Ślęża", "Skopiec", "Kłodzka Góra", "Chełmiec", "Biskupia Kopa", "Szczeliniec Wielki", "Waligóra", "Skalnik", "Jagodna", "Kowadło", "Wielka Sowa", "Orlica", "Rudawiec", "Wysoka Kopa", "Śnieżnik", "Śnieżka"]
        ...
        if str(ach.achievement_id) == "16fa5a46-b2a8-4bae-afe8-c69a2e1d6181":# Powyżej 1000 m p.w.m.
            ...
            if counter == len(above_thousand):
                achievements_to_add.append(models.Users_Achievements(user_id=user_id, achievement_id=ach.achievement_id))          
        if str(ach.achievement_id) == "16fa5a46-b2a8-4bae-afe8-c69a2e1d6181":# Poniżej 1000 m p.w.m.
            # below_thousand = ["Lackowa", "Łysica", "Biskupia Kopa", "Skopiec", "Lubomir", "Kłodzka Góra", "Waligóra", "Kowadło", "Szczeliniec Wielki", "Chełmiec", "Ślęża", "Skalnik", "Jagodna", "Czupel"]
            below_thousand = ['0272834d-2a43-411a-bfb1-b90239513491', 'cfe4d9bc-6790-4268-802a-47f48a3904cc', '3bd597dd-95ec-4b6a-b7f1-7b2370c65ca3',
                            '685d2ab6-221c-4659-b146-5e240fa22a8e', 'c97af7f6-8d36-4674-8d6e-18a7023cfef8', '2bb31a0b-2332-48ec-8676-2331e2f9cad0',
                            '443fde7d-5958-45c1-9d97-70368e5769e4', 'b939a070-af06-4831-9727-14421c269a10', 'fbc30a1e-4a39-436c-bc94-44fd2452d54e',
                            '990b80e3-1e90-48f0-a78f-8871580f7a09', 'e5dc6302-5f16-4c71-a282-ecf93393efaa', 'f54d011e-7d27-4b9d-9b83-211d532d84e3',
                            '9a849d45-5b6e-417f-8e49-08ea9d4e75b9', '0203d577-a491-4e55-8f0f-8ffeffe20d82']
            counter = 0
            for mountain in user_mountains:
